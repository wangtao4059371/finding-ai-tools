...
const MODELS = [
  ...
  {nm:"Gemini 3.5 Flash",co:"Google DeepMind（美国）",pr:"输入$1.50/M, 输出$9.00/M tokens",ur:"https://deepmind.google/gemini",de:"在2026年5月19日的Google I/O大会上，DeepMind正式发布了Gemini 3.5 Flash，并跳过预览阶段直接进入商用状态。Google将其确立为全球Google Search AI Mode和Gemini官方应用的核心默认模型，可见内部对其能力的信心。3.5 Flash默认开启中等强度的内部思考机制，模型在给出最终答案前会自动在后台运行多步推理，关键是这种中间推理状态能在多轮对话中被自动保留——即使连续对话几十轮，核心逻辑推导也不会丢失。这让它在Agent场景下异常可靠。性能方面，3.5 Flash号称4倍速低延迟，处理数万字长文本和调用外部工具时几乎瞬时响应。另一个差异化能力是原生云端长时运行——配合Gemini Spark智能体架构，开发者可以将其设为离线全天候执行复杂工作流，例如自动扫描多日邮件、汇总数据并生成报告。1M token上下文窗口支持文本、图像、音频、视频四种输入模态，输出上限65K token。价格方面，输入每百万token仅1.5美元，输出9美元，缓存输入更是低至0.15美元。综合来看，除了API成本因性能暴涨比上一代略高以外，它在编程、Agent编排、推理和响应速度四个维度上形成了极强的统治力，彻底模糊了轻量模型与旗舰模型的边界。",re:9.2,ch:8.5,coding:9.5,ma:8.8,ha:9.0,sp:9.8,ap:5.0,ag:9.6,lt:9.0},
